...
from pylowl.proj.brightside.hdp.core import *
...
DEFAULT_OUTPUT_PARENT_DIR = 'output/pylowl/proj/brightside/hdp'
...
def run(**kwargs):
    ...
    logging.info("Creating online hdp instance")
    ...
    save_global(m, 'init', output_dir, options['save_model'])
    ...
    while True:
        ...
        if total_doc_count % options['save_lag'] == 0:
            ...
            if options['test_data_dir'] is not None:
                test_hdp_predictive(m, c_test_train, c_test_test, batchsize, options['var_converge'], options['test_samples'])
