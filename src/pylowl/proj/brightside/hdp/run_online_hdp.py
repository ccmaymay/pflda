...
def parse_args():
    parser = OptionParser()
    parser.set_defaults(K=300, T=20, D=-1, W=-1, eta=0.01, alpha=1.0, gamma=1.0,
                        ...
                        test_data_path_in_folds=None, directory='output', save_lag=500, pass_ratio=0.5,
...
def run_online_hdp():
    ...
    if options.test_data_path_in_folds is not None:
        ...
        c_test_train_folds = [read_data(filename)
                              for filename in test_data_train_filenames]
        c_test_test_folds = [read_data(filename)
                             for filename in test_data_test_filenames]
    ...
    ohdp = online_hdp.online_hdp(options.K, options.T, options.D, options.W,
                                options.eta, options.alpha, options.gamma,
                                options.kappa, options.tau, options.scale,
                                options.adding_noise)
    if options.new_init:
        ohdp.new_init(c_train, options.burn_in_samples)
    ...
    while True:
        ...
        if total_doc_count % options.save_lag == 0:
            ...
            if options.save_model:
                ohdp.save_topics('%s/doc_count-%d.topics' %
                                 (result_directory, total_doc_count))
                cPickle.dump(ohdp, file('%s/doc_count-%d.model' %
            ...
            if options.test_data_path is not None:
                ...
                for doc in c_test.docs:
                    (likelihood, gamma) = online_hdp.lda_e_step(
                        doc, lda_alpha, lda_beta)
                    test_score += likelihood
                    (likelihood, count, gamma) = online_hdp.lda_e_step_split(
                        doc, lda_alpha, lda_beta)
