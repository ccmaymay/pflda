...
\begin{itemize}
\item $q(x_d) = \Discrete(x_d \mid \xi_d)$
...
\begin{align*}
    &\Eq \log \Pr(x_d \mid p) \\
    &= \sum_m \xi_{dm} \Eq \log p_m
\end{align*}

\begin{align*}
    &\Eq \log q(x_d) \\
    &= \sum_m \xi_{dm} \log \xi_{dm}
\end{align*}
...
\begin{align*}
    ...
    &= \sum_j \varphi_i^{(d)}(j) \left[ \sum_m \xi_{dm} \left[ \Eq \log V^{(m)}_j + \sum_{j'<j} \Eq \log (1-V^{(m)}_{j'}) \right] \right]
\end{align*}
...
\begin{align*}
    ...
    &= \sum_i \nu_{dn}(i) \sum_j \varphi_i^{(d)}(j) \sum_m \xi_{dm} \sum_k \zeta_j^{(m)}(k) \Eq \log \theta_{k W_{dn}}
\end{align*}
...
\begin{align*}
    ...
    &\qquad + \sum_d \sum_n \delta_{W_{dn}}(w) \sum_i \nu_{dn}(i) \sum_j \varphi_i^{(d)}(j) \sum_m \xi_{dm} \zeta_j^{(m)}(k) \left[ \trigamma{\lambda_{k w}} - \trigamma{\sum_{w'}{\lambda_{kw'}}} \right] \\
    &\Rightarrow
    \boxed{ \lambda_{kw} = \lambda_0 + \sum_d \sum_n \delta_{W_{dn}}(w) \sum_i \nu_{dn}(i) \sum_j \varphi_i^{(d)}(j) \sum_m \xi_{dm} \zeta_j^{(m)} }.
\end{align*}
...
\begin{align*}
    ...
    &\qquad + \sum_d \left[ \xi_{dm} \trigamma{\omega_m} - \trigamma{\sum_{m'} \omega_{m'}} \sum_{m'} \xi_{dm'} \right] \\
    &\Rightarrow
    \boxed{ \omega_m = \sum_m \xi_{dm} }.
\end{align*}
...
\begin{align*}
    ...
    &\qquad + \sum_d \xi_{dm} \sum_i \left[ \varphi_i^{(d)}(j) \left[ \trigamma{a_j^{(m)}} - \trigamma{a_j^{(m)} + b_j^{(m)}} \right] - \sum_{j'>j} \varphi_i^{(d)}(j') \trigamma{a_j^{(m)} + b_j^{(m)}} \right] \\
    &\Rightarrow
    \boxed{ a_j^{(m)} = 1 + \sum_d \xi_{dm} \sum_i \varphi_i^{(d)}(j) }.
\end{align*}
...
\begin{align*}
    ...
    &\qquad + \sum_d \xi_{dm} \sum_i \left[ - \varphi_i^{(d)}(j) \trigamma{a_j^{(m)} + b_j^{(m)}} + \sum_{j'>j} \varphi_i^{(d)}(j') \left[ \trigamma{b_j^{(m)}} + \trigamma{a_j^{(m)} + b_j^{(m)}} \right] \right] \\
    &\Rightarrow
    \boxed{ b_j^{(m)} = \beta + \sum_d \xi_{dm} \sum_i \sum_{j' > j} \varphi_i^{(d)}(j') }.
\end{align*}

\begin{align*}
    &\pd{\zeta_j^{(m)}} \Eq \left[ \log \Pr(y_j^{(m)} \mid V_\cdot) - \log q(y_j^{(m)}) + \sum_d \sum_n \log \Pr\left(W_{dn} \mid c_{dn}, z_\cdot^{(d)}, x_d, y_\cdot^{(\cdot)}, \theta_{\cdot W_{dn}}\right) \right] \\
    &= \sum_k \left[ \Eq \log V_k + \sum_{k'<k} \Eq \log (1-V_{k'}) + \sum_d \xi_{dm} \sum_n \sum_i \nu_{dn}(i) \varphi_i^{(d)}(j) \Eq \log \theta_{k W_{dn}} \right] \\
    &\qquad - \sum_k \left[ 1 + \log \zeta_j^{(m)}(k) \right] \\
    &\Rightarrow
    \boxed{ \zeta_j^{(m)}(k) \propto \exp\left[ \Eq \log V_k + \sum_{k'<k} \Eq \log (1-V_{k'}) + \sum_d \xi_{dm} \sum_n \sum_i \nu_{dn}(i) \varphi_i^{(d)}(j) \Eq \log \theta_{k W_{dn}} \right] } .
\end{align*}

\begin{align*}
    &\pd{\xi_{d\cdot}} \Eq \left[ \log \Pr(x_d \mid p) - \log q(x_d) + \sum_d \sum_i \log \Pr(z_i^{(d)} \mid V^{(m=\cdot)}_\cdot, x_d) \right] \\
    &= \sum_m \left[ \Eq \log{p_m} + \sum_d \sum_i \sum_j \varphi_i^{(d)}(j) \left[ \Eq \log V^{(m)}_j + \sum_{j'<j} \Eq \log (1-V^{(m)}_{j'}) \right] \right] \\
    &\qquad - \sum_m \left[ 1 + \log \xi_{dm} \right] \\
    &\Rightarrow
    \boxed{ \xi_{dm} \propto \exp\left[ \Eq \log{p_m} + \sum_d \sum_i \sum_j \varphi_i^{(d)}(j) \left[ \Eq \log V^{(m)}_j + \sum_{j'<j} \Eq \log (1-V^{(m)}_{j'}) \right] \right] }.
\end{align*}
...
\begin{align*}
    ...
    &= \sum_j \sum_m \xi_{dm} \left[ \Eq \log V_j^{(m)} + \sum_{j'<j} \Eq \log (1-V_{j'}^{(m)}) + \sum_n \nu_{dn}(i) \sum_k \zeta_j^{(m)}(k) \Eq \log \theta_{k W_{dn}} \right] \\
    &\qquad - \sum_j \left[ 1 + \log \varphi_i^{(d)}(j) \right] \\
    &\Rightarrow
    \boxed{ \varphi_i^{(d)}(j) \propto \exp\left[ \sum_m \xi_{dm} \left[ \Eq \log V_j^{(m)} + \sum_{j'<j} \Eq \log (1-V_{j'}^{(m)}) + \sum_n \nu_{dn}(i) \sum_k \zeta_j^{(m)}(k) \Eq \log \theta_{k W_{dn}} \right] \right] } .
\end{align*}
...
\begin{align*}
    ...
    &= \sum_i \left[ \Eq \log V_i^{(d)} + \sum_{i'<i} \Eq \log (1-V_{i'}^{(d)}) + \sum_j \varphi_i^{(d)}(j) \sum_m \xi_{dm} \sum_k \zeta_j^{(m)}(k) \Eq \log \theta_{k W_{dn}} \right] \\
    &\qquad - \sum_i \left[ 1 + \log \nu_{dn}(i) \right] \\
    &\Rightarrow
    \boxed{ \nu_{dn}(i) \propto \exp\left[ \Eq \log V_i^{(d)} + \sum_{i'<i} \Eq \log (1-V_{i'}^{(d)}) + \sum_j \varphi_i^{(d)}(j) \sum_m \xi_{dm} \sum_k \zeta_j^{(m)}(k) \Eq \log \theta_{k W_{dn}} \right] } .
\end{align*}
