...
\begin{align*}
    ...
    &= \sum_m \left[ \digamma{\omega_m} - \digamma{\sum_{m'} \omega_{m'}} + \sum_d \sum_i \sum_j \varphi_i^{(d)}(j) \left[ \Eq \log V^{(m)}_j + \sum_{j'<j} \Eq \log (1-V^{(m)}_{j'}) \right] \right] \\
    &\qquad - \sum_m \left[ 1 + \log \mu_{dm} \right] \\
    &\Rightarrow
    \boxed{ \mu_{dm} \propto \exp\left[ \digamma{\omega_m} - \digamma{\sum_{m'} \omega_{m'}} + \sum_d \sum_i \sum_j \varphi_i^{(d)}(j) \left[ \Eq \log V^{(m)}_j + \sum_{j'<j} \Eq \log (1-V^{(m)}_{j'}) \right] \right] }.
\end{align*}
